...
class SettingsTableViewController: UITableViewController {
    ...
    let disclaimerFont = UIFont.preferredFont(forTextStyle: .footnote)
    ...
    func getDisclaimerSizeForString(string: String!) -> CGSize {
        ...
        return disclaimerString.boundingRect(with: CGSize(width: self.tableView.frame.width-20,
                                             ...
                                             attributes: [ NSAttributedStringKey.font: disclaimerFont ],
                                             context: nil).size
    }
    ...
    @objc func reloadSelectedRowOrTableView() {
        if let selectedRow = self.tableView.indexPathForSelectedRow {
            self.tableView.reloadRows(at: [selectedRow], with: UITableViewRowAnimation.none)
            self.tableView.selectRow(at: selectedRow, animated: false, scrollPosition: UITableViewScrollPosition.none)
            self.tableView.deselectRow(at: selectedRow, animated: true)
        } else {
